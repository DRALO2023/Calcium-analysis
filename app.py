...
def auc_above_baseline(x, y):
    y_pos = np.maximum(y, 0)
    return float(np.trapezoid(y_pos, x)) if hasattr(np, "trapezoid") else float(np.trapz(y_pos, x))


def first_drop_below_zero(x, y):
...
if control_file and treatment_file:
    ...
    if control_cell_cols and treatment_cell_cols:
        ...
        if add_stats:
            ...
            st.plotly_chart(build_single_plot_with_sem(control_avg, sem_control, "Control", "#1f77b4"), use_container_width=True)
            st.subheader("Average trace: Treatment (with SEM)")
            st.plotly_chart(build_single_plot_with_sem(treatment_avg, sem_treatment, "Treatment", "#d62728"), use_container_width=True)
            st.subheader("Overlay: Control vs Treatment (with SEM)")
            st.plotly_chart(build_overlay_plot_with_sem(control_avg, treatment_avg, sem_control, sem_treatment), use_container_width=True)
        else:
